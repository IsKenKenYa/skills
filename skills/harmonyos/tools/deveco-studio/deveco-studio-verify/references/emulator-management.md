...
> **⚠️ 警告：强制停止将丢失未保存状态**
> 强制停止模拟器会**立即终止所有正在运行的进程**。模拟器中任何**未保存的状态、正在进行的调试会话或应用数据都将丢失**。请优先改用优雅的关闭方法（如方法1的 DevEco Studio 界面关闭按钮，或先 `hdc shell aa force-stop <bundle>` 停止应用后再正常退出），仅在其他方法无效时才使用强制关闭。
...
> **⚠️ 注意：删除操作永久且不可逆**
> 删除镜像是**永久性操作，无法撤销**，会永久删除镜像文件及相关数据。删除前请确认你不再需要该镜像；如有重要的测试配置依赖该镜像，请先备份再执行删除。
...
> **⚠️ 注意：删除模拟器实例和镜像是永久性、不可逆操作**
> `emulator -delete`（删除实例）和 `emulator -imageDelete`（删除镜像）的操作**永久且无法撤销**，会永久删除相关数据。清理前请确认你不再需要该实例或镜像；如有重要的测试配置，请先备份，再执行删除。
