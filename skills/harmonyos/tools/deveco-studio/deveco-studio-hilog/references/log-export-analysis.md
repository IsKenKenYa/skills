...
> **警告：导出的日志包可能包含敏感/私人数据**
> `-logZip` 导出的完整日志包可能包含敏感与私人数据，包括：
> - **用户信息**：账号、手机号、邮箱、用户标识符等
> - **应用状态**：登录令牌、API 密钥、凭据、业务数据
> - **系统详细信息**：设备标识、系统版本、配置参数

**数据处理建议**：

1. **外部分享前先审查**：共享日志前逐文件检查是否含敏感信息，再决定是否共享。
2. **发布前清理敏感数据**：将日志发布到错误报告（Issue）或论坛前，对敏感片段进行脱敏（打码/替换/删除整行），仅保留与问题相关的最小片段。
3. **存储在安全的非共享目录**：将导出的日志包保存到本机私有、非共享目录，分析完成后及时删除，避免长期留存敏感数据。
4. **内核与内存日志尤其敏感**：`kernel.log` 与 `memInfo.log` 尤其可能暴露**系统级信息**（内核地址、内存布局、进程结构等），共享前需重点审查或直接排除。
...
9. **隐私与数据处理**：导出的日志包可能含敏感数据（用户信息、凭据、系统信息）。共享/发布前审查并脱敏，存于私有非共享目录，分析后删除；`kernel.log`/`memInfo.log` 尤其敏感，需重点审查（详见上方「隐私与数据处理警告」）
