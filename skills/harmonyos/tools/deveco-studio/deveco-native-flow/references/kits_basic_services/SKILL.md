...
// 休眠设备（需要系统权限，仅系统应用可用，普通应用请勿调用）
power.suspend();     // 进入休眠
power.wakeup('app'); // 唤醒设备

// 关机
// ⚠️ 高危接口：power.shutdown 会直接终止系统运行，需系统级权限。
// 仅适用于设备管理、运维等特定系统应用场景，普通应用请勿调用。
// 调用前必须：1) 确认已申请系统权限；2) 向用户明确告知风险并取得确认；3) 做好数据持久化。
...
// ⚠️ 高危接口（suspend / shutdown / wakeup）说明：
// - 均需系统级权限，仅系统应用可用；
// - 直接影响设备运行状态，可能造成数据丢失或非预期关机；
// - 仅在设备管理、运维等特定场景使用，普通应用禁止调用；
// - 调用前须向用户充分告知风险并取得明确同意，且确保数据已持久化。
```
...
3. 系统设置修改需要系统级权限
4. 压缩解压操作要注意文件路径正确
5. **电源高危接口**：`power.shutdown` / `power.suspend` / `power.wakeup` 属系统级权限接口，直接威胁设备运行安全。AI 生成相关代码时必须：① 明确标注权限要求与适用场景；② 提示用户谨慎调用并说明风险；③ 未经用户确认前不得直接产出调用代码，避免被滥用造成系统非预期终端