...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b3/v3/UKSph-PiSqS8KKEAxz0VfA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T104853Z&HW-CC-Expire=86400&HW-CC-Sign=AB978D7A35FB1C7576F3B94EF07B0272DF5A64235E9EDFE277245EBDFF288295)
为了保证应用的运行效率，大部分API调用都是异步的，对于异步调用的API，均提供了callback和Promise两种方式，以下示例均采用promise函数，更多方式可以查阅 [@ohos.net.connection (网络连接管理)](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 。
#### 基本概念
...
此权限级别为normal，在申请权限前，请保证符合 [权限使用的基本原则](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/安全/程序访问控制/应用权限管控/应用权限管控概述/app-permission-mgmt-overview.md) 。然后参考 [访问控制-声明权限](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/安全/程序访问控制/应用权限管控/申请应用权限/声明权限/declare-permissions.md) 声明对应权限。
2.
...
```
3.
调用 [createNetConnection](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，指定网络能力、网络类型和超时时间(可选，如不传入代表默认网络；创建不同于默认网络时可通过指定这些参数完成)，创建一个NetConnection对象。
```
...
```
4.
调用该对象的 [on()](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，传入type和callback，订阅关心的事件。
```
...
```
5.
调用该对象的 [register()](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，订阅指定网络状态变化的通知。当网络可用时，会触发netAvailable事件的回调；当网络从连接到断开时，会触发netLost事件的回调；当网络连接信息变化时（例如linkAddresses增加V6地址），会触发netConnectionPropertiesChange事件回调；当网络能力发生变化时（例如网络的连通性发生变化），会触发netCapabilitiesChange回调。
```
// 订阅连接状态变化
...
```
6.
当不使用该网络时，可以调用该对象的 [unregister()](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，取消订阅。
```
...
2.
查询默认网络或指定网络连接信息代码示例
通过调用 [getDefaultNet](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取默认的数据网络(NetHandle)；调用 [getNetCapabilities](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取该NetHandle对应网络的能力信息。能力信息包含了网络类型(蜂窝网络、Wi-Fi网络、以太网网络等)、网络具体能力等网络信息。也可以调用 [getConnectionProperties](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取该NetHandle对应网络的连接信息。
```
// 从@kit.NetworkKit中导入connection命名空间。
import { connection } from '@kit.NetworkKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
3.
查询所有网络连接信息代码示例
通过调用 [getAllNets](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取所有处于连接状态的网络列表(Array<NetHandle>)。然后遍历获取到的NetHandle数组，分别调用 [getNetCapabilities](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取该NetHandle对应网络的能力信息，能力信息包含了网络类型(蜂窝网络、Wi-Fi网络、以太网网络等)、网络具体能力等网络信息。也可以调用 [getConnectionProperties](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取该NetHandle对应网络的连接信息。
```
// 从@kit.NetworkKit中导入connection命名空间。
import { connection } from '@kit.NetworkKit';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
2.
代码示例
调用 [getDefaultNetSync](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取当前默认网络的netHandle，netHandle有效的情况下，调用 [getNetCapabilitiesSync](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，获取NetHandle对应网络的能力信息，根据获取到的能力信息，判断networkCap数组中的值判断网络是否可用。
...
2.
代码示例
调用 [getAddressesByName](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Network Kit（网络服务）/ArkTS API/js-apis-net-connection.md) 方法，使用默认网络解析主机名以获取所有IP地址。
```
// 从@kit.NetworkKit中导入connection命名空间。
import { connection } from '@kit.NetworkKit';
import { BusinessError } from '@kit.BasicServicesKit';
