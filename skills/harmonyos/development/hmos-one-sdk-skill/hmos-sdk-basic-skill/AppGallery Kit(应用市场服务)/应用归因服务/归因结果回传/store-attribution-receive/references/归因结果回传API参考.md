...
| trigger_data | number | 否 | 是 | 转化事件编码。[标准转化事件](D:/code/APIDevice/output/md_output/harmonyos-guides/应用服务/AppGallery Kit（应用市场服务）/应用归因服务/开发准备/管理转化事件/标准转化事件/store-attribution-trigger-standard.md)取值范围：[1, 200]。[自定义转化事件](D:/code/APIDevice/output/md_output/harmonyos-guides/应用服务/AppGallery Kit（应用市场服务）/应用归因服务/开发准备/管理转化事件/自定义转化事件/store-attribution-trigger-custom.md)取值范围：[501, 600]。 |
...
| signature | String | 是 | 否 | 签名值。接收方应使用应用归因服务提供的[公钥](D:/code/APIDevice/output/md_output/harmonyos-guides/应用服务/AppGallery Kit（应用市场服务）/应用归因服务/归因结果回传/store-attribution-receive.md)，对该签名值进行验签，具体参考[验签计算规则](D:/code/APIDevice/output/md_output/harmonyos-guides/应用服务/AppGallery Kit（应用市场服务）/应用归因服务/归因结果回传/store-attribution-receive.md)、[回传结果验签方法](#section114341820105218)。 |
...
#### 回传结果验签方法
1.
[Bouncy Castle](https://www.bouncycastle.org/download/bouncy-castle-java/)
```java
<dependency>
...
```
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c9/v3/6Jvon5vhQ9uW2CgxfPTxhQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093802Z&HW-CC-Expire=86400&HW-CC-Sign=A30BEDAF79E0F4E5E0178162A61A0B8EC69EABF38CF20FB8CD99842EE3CE9970)
回传结果验签调用方法verify(String content,String signature)中：
content：签名内容拼接字段，拼接方式可参考 [验签计算规则](D:/code/APIDevice/output/md_output/harmonyos-guides/应用服务/AppGallery Kit（应用市场服务）/应用归因服务/归因结果回传/store-attribution-receive.md) 。
signature：回传请求中的签名值。