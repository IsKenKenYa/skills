...
- [扫描管控](#扫描管控)
#### 接口说明
完整的JS API说明以及示例代码请参考： [SCAN接口](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-wifimanager.md) 。
接口具体说明如下表所示。
| 接口名 | 功能描述 |
| --- | --- |
...
5.
示例代码：
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/70/v3/9qxb2nEfT1iko_u__cagIw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T104848Z&HW-CC-Expire=86400&HW-CC-Sign=CDDE8E0762B1F45FF3DD600D47351AF95AC58C8C1DCC60E7D96B599AD1065199)
...
```
6.
错误码详情请参见 [WIFI错误码](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/错误码/errorcode-wifi.md) 。
#### PNO扫描
...
触发场景描述：
1.
设备亮屏且与Wi-Fi已连接：判断当前链路是否支持上网。支持上网，扫描间隔1h；不支持上网，判断是否静止，静止场景下扫描周期从20s开始倍增，最大300s；非静止场景扫描周期从20s开始，最大160s。
2. 设备亮屏且与Wi-Fi未连接：静止场景扫描周期从20s开始倍增，最大300s；非静止场景扫描周期从20s开始，最大160s。
#### 扫描管控
WiFi扫描管控是指对无线设备扫描WiFi网络的行为进行管理和控制。
以下是一些常见的管控方式和应用场景：
1.
WiFi在关闭的情况下不触发周期扫描和PNO扫描。
2.
WiFi连接过程中不允许触发扫描。
3.
前台应用2分钟内最多发起4次扫描。
...
设备温度达到阈值，设备扫描会被管控。
5.
扫描接口从API version 9开始支持，从API version 10开始废弃，替代接口仅向系统应用开放。
