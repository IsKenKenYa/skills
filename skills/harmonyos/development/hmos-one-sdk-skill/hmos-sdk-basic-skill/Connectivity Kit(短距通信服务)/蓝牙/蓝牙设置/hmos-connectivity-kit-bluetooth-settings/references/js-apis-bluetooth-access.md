...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/3d/v3/gF1FKTK9QF-jxQjxsGmwfA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093249Z&HW-CC-Expire=86400&HW-CC-Sign=AC4924F9B3558456AAD848FABD432C64F45D3E74FD5A2C3C23DD14BBD6C51B2C)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 事件回调类型，支持的事件为'stateChange'，表示蓝牙开关状态变化事件。如：当调用[access.enableBluetooth](#accessenablebluetooth)或[access.disableBluetooth](#accessdisablebluetooth)时，可触发该事件。 |
...
| callback | Callback<[BluetoothState](#bluetoothstate)> | 否 | 指定取消订阅的回调函数通知。若传参，则需与[access.on('stateChange')](#accessonstatechange)中的回调函数一致；若无传参，则取消订阅该type对应的所有回调函数通知。 |
**错误码** ：
以下错误码的详细介绍请参见 [通用错误码说明文档](D:/code/APIDevice/output/md_output/harmonyos-references/API参考概述/errorcode-universal.md) 和 [蓝牙服务子系统错误码](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/错误码/errorcode-bluetoothmanager.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceId | string | 是 | 对端设备的虚拟MAC地址，例如："XX:XX:XX:XX:XX:XX"。该地址一般来源于蓝牙扫描结果，如：可通过调用[startScan](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-ble.md)或[connection.startBluetoothDiscovery](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md)扫描得到。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象，无返回结果。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| deviceId | string | 是 | 对端设备的虚拟MAC地址，例如："XX:XX:XX:XX:XX:XX"，该地址一般来源于蓝牙扫描结果，如：通过调用[startScan](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-ble.md)或[connection.startBluetoothDiscovery](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md)扫描得到。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象，无返回结果。 |
...
- 若输入不符合以上格式的UUID或包含非16进制范围内的字符，将返回[401](D:/code/APIDevice/output/md_output/harmonyos-references/API参考概述/errorcode-universal.md)错误码。
**系统能力** ：SystemCapability.Communication.Bluetooth.Core
**参数：**
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
