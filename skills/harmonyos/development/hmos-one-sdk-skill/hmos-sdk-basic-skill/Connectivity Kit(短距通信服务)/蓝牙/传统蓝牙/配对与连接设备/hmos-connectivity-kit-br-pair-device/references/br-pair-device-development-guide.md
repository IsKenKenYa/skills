...
```
#### 订阅配对状态变化事件
通过订阅配对状态变化事件，可以获取实时的配对状态。在整个配对过程中，涉及多种状态的跃迁，其中 [BOND_STATE_BONDED](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md) 表示已配对。
...
```
#### 发起配对
若目标设备的配对状态是 [BOND_STATE_INVALID](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md) ，则可以主动配对目标设备。
- 目标设备可以通过发现设备流程获取，详情请参考：[传统蓝牙查找设备](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/网络/Connectivity Kit（短距通信服务）/蓝牙/传统蓝牙/查找设备/br-discovery-development-guide.md)或者[低功耗蓝牙查找设备](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/网络/Connectivity Kit（短距通信服务）/蓝牙/低功耗蓝牙/查找设备/ble-development-guide.md)。
配对过程中，系统会弹出对话框。不同配对类型，对话框样式可能不一样，其中“确认配对密钥（Confirm Passkey）”模式如下图1。若用户同意授权，才能配对成功。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/93/v3/PfW6Ib5uSkCv5ZYCaSLsEw/zh-cn_image_0000002628861014.png?HW-CC-KV=V1&HW-CC-Date=20260701T104846Z&HW-CC-Expire=86400&HW-CC-Sign=A6E9712DE3F10D1F53C9540CBA8AA2A70F9EF985991321AA596F3946C235CECD)
**图1** 蓝牙配对请求对话框
蓝牙设备的实际MAC地址属于用户的隐私信息，在发现设备过程中，蓝牙子系统会给每个蓝牙外设分配一个虚拟MAC地址，并保存该虚拟MAC地址与外设实际MAC地址的映射关系。
若开发者不知道目标设备的 [地址类型](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-common.md) ，推荐使用API version 20及以前的配对方式发起配对，详情请见 [connection.pairDevice](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md) 。
...
```
若开发者已知目标设备的 [地址类型](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-common.md) ，推荐使用API version 21开始支持的配对方式发起配对，详情请见 [connection.pairDevice](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md) 。
...
```
#### 连接已配对设备的profile
若应用配对完目标设备后，可以调用 [connectAllowedProfiles](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md) ，发起连接该设备支持的profile能力（只包括A2DP、HFP和HID）。若应用需要使用SPP连接，请参考 [连接和传输数据](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/网络/Connectivity Kit（短距通信服务）/蓝牙/传统蓝牙/连接和传输数据/spp-development-guide.md) 。
- 蓝牙子系统会在配对过程中查询和保存目标设备支持的所有profile能力。
- 配对完成后，应用可以主动查询目标设备的profile能力，需调用[getRemoteProfileUuids](D:/code/APIDevice/output/md_output/harmonyos-references/系统/网络/Connectivity Kit（短距通信服务）/ArkTS API/js-apis-bluetooth-connection.md)。若存在应用需要的能力，则可以在配对完成后30s内，发起连接目标设备的profile。
```
// 设备地址是已配对的设备
