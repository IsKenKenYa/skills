...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| scanType | scanCore.[ScanType](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-scancore.md) | 否 | 否 | 码类型。 |
...
| level | [ErrorCorrectionLevel](#section1829612519455) | 否 | 是 | 纠错水平，默认值为LEVEL_H。此参数只在生成QR码时有效。 |
| backgroundColor | number | 否 | 是 | 生成码图背景颜色，HEX格式颜色，默认为白色（0xFFFFFF）。 |
| pixelMapColor | number | 否 | 是 | 生成码图颜色，HEX格式颜色，默认为黑色（0x000000）。 |
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/53/v3/_f3K2pDYQx6t92XjG0-M9A/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093647Z&HW-CC-Expire=86400&HW-CC-Sign=86B8D09C23A30BCE011E1EF3A072170862B71FD302B38340E2439690596EE536)
生成码参数建议：
-
...
| options | [CreateOptions](#section87678254617) | 是 | 用于设置生成码图的参数。 |
**返回值：**
| **类型** | **说明** |
| --- | --- |
| Promise<image.[PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)> | Promise对象，返回生成的码图对象。 |
**错误码：**
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| content | string | 是 | 码内容字符串。参数限制请参见[generateBarcode.createBarcode](#section1841142919352)的表1-content参数限制条件。 |
| options | [CreateOptions](#section87678254617) | 是 | 用于设置生成码图的参数。 |
| callback | AsyncCallback<image.[PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)> | 是 | 回调函数。当码图生成成功，err为undefined，data为生成的码图对象image.PixelMap；否则为错误对象。 |
**错误码：**
以下错误码的详细介绍请参见 [ArkTS API错误码](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-error-code.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
...
| options | [CreateOptions](#section87678254617) | 是 | 用于设置生成码图的参数，ScanType仅支持QR Code。 |
**返回值：**
| **类型** | **说明** |
| --- | --- |
| Promise<image.[PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)> | Promise对象，返回生成的码图对象。 |
