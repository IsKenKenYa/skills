...
| **名称** | **类型** | 只读 | **可选** | **说明** |
| --- | --- | --- | --- | --- |
| scanType | scanCore.[ScanType](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-scancore.md) | 否 | 否 | 码类型。**元服务API：**从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| originalValue | string | 否 | 否 | 码识别内容结果。**元服务API：**从版本4.1.0(11)开始，该接口支持在元服务中使用。 |
| scanCodeRect | [ScanCodeRect](#section1456184716188) | 否 | 是 | 码识别位置信息。**起始版本：**4.1.0(11) |
| cornerPoints | Array<[Point](#section9634457911)> | 否 | 是 | 码识别角点位置信息，返回QR Code四个角点。此参数仅图像识码接口[detectBarcode.decodeImage](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-imagedecode.md)返回。**起始版本：**5.0.0(12) |
...
| source | scanCore.[ScanSource](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-scancore.md) | 否 | 是 | 扫码结果来源。此参数仅默认界面扫码接口返回。**模型约束：**此接口仅可在Stage模型下使用。**元服务API：**从版本6.0.2(22)开始，该接口支持在元服务中使用。**起始版本：**6.0.2(22) |
#### ScanCodeRect
...
自定义界面扫码返回的坐标单位与传入参数 [ViewControl](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-customscan-api.md) 中width、height单位一致。
#### Point
...
| **名称** | **类型** | 只读 | **可选** | **说明** |
| --- | --- | --- | --- | --- |
| scanTypes | Array<scanCore.[ScanType](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-scancore.md)> | 否 | 是 | 设置扫码类型，默认扫码ALL（全部码类型）。 |
...
| **参数名** | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| context | [common.Context](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/js-apis-app-ability-common.md) | 是 | 应用上下文。 |
| options | [ScanOptions](#section1285191073117) | 否 | 启动扫码参数。 |
**返回值：**
| **类型** | **说明** |
| --- | --- |
| Promise<[ScanResult](#section10614317162112)> | Promise对象，返回扫码结果对象。 |
**错误码：**
...
| --- | --- | --- | --- |
| context | [common.Context](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/js-apis-app-ability-common.md) | 是 | 应用上下文。 |
| callback | AsyncCallback<[ScanResult](#section10614317162112)> | 是 | 回调函数。当扫码返回成功，err为undefined，data为获取到的扫码结果对象[ScanResult](#section10614317162112)；否则为错误对象。 |
**错误码：**
以下错误码的详细介绍请参见 [ArkTS API错误码](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-error-code.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
...
| --- | --- | --- | --- |
| context | [common.Context](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/js-apis-app-ability-common.md) | 是 | 应用上下文。 |
| options | [ScanOptions](#section1285191073117) | 是 | 启动扫码参数。 |
| callback | AsyncCallback<[ScanResult](#section10614317162112)> | 是 | 回调函数。当扫码返回成功，err为undefined，data为获取到的扫码结果对象[ScanResult](#section10614317162112)；否则为错误对象。 |
**错误码**
以下错误码的详细介绍请参见 [ArkTS API错误码](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-error-code.md) 。
| 错误码ID | 错误信息 |
...
**废弃说明：** 从版本4.1.0(11)开始废弃，建议使用 [startScanForResult](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Scan Kit（统一扫码服务）/ArkTS API/scan-scanbarcode-api.md) 替代。
**系统能力：** SystemCapability.Multimedia.Scan.ScanBarcode
**起始版本：** 4.0.0(10)
**参数：**
| **参数名** | **类型** | 必填 | **说明** |
