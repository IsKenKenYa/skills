...
获取用户点击事件需使用 [voipCall.on](#section7587131203319) 在业务流程开始时提前订阅voipCallUiEvent事件，可于业务流程结束后使用 [voipCall.off](#section84793361325) 结束订阅。
**起始版本：** 4.1.0(11)
#### 导入模块
```typescript
...
| voipCallUiEvent | [VoipCallUiEvent](#section26961727970) | 否 | 否 | 应用内通话事件。 |
| errorReason | [ErrorReason](#section15252018876) | 否 | 否 | 应用内通话错误码。 |
...
| --- | --- | --- | --- | --- |
| callId | string | 否 | 否 | 应用内通话唯一ID。 |
| voipCallType | [VoipCallType](#section0933646875) | 否 | 否 | 应用内通话类型。 |
| userName | string | 否 | 否 | 应用内通话用户昵称。 |
| userProfile | image.[PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md) | 否 | 否 | 用户头像图片，需导入图像处理模块，详情见[PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)。支持传入的最大图片大小为 221 x 221 像素，推荐传入的图片大小为 112 x 112 像素。通过[PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)的[getPixelBytesNumber](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)接口获取到的图片大小要小于196608。 |
| abilityName | string | 否 | 否 | 接听后需加载的应用界面ability名称。 |
| voipCallState | [VoipCallState](#section24523814719) | 否 | 否 | 应用内通话状态。 |
...
| callback | Callback<[VoipCallUiEventInfo](#section1178710819715)> | 是 | 回调函数，返回通话事件详细信息对象。 |
**错误码：**
以下错误码的详细介绍请参见 [ArkTS API错误码](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Call Service Kit（通话服务）/ArkTS API/call-error-code.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
| 401 | Parameter error. |
...
| --- | --- | --- | --- |
| type | string | 是 | 通话事件类型，参数固定为“voipCallUiEvent”。 |
| callback | Callback<[VoipCallUiEventInfo](#section1178710819715)> | 否 | 需要取消监听的回调函数，返回通话事件详细信息对象。若不填，则取消当前应用监听该事件的所有回调函数。 |
**错误码：**
以下错误码的详细介绍请参见 [ArkTS API错误码](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Call Service Kit（通话服务）/ArkTS API/call-error-code.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
...
通知来电消息，如果应用来电消息建立失败，需调用 [reportIncomingCallError](#section155796451716) 通知来电建立失败。需设置通话详细信息，见 [VoipCallAttribute](#section4221135015444) 。使用Promise异步回调。
**系统能力** ：SystemCapability.Telephony.VoipCallManager
**起始版本：** 4.1.0(11)
**参数：**
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | [VoipCallAttribute](#section4221135015444) | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见[VoipCallAttribute](#section4221135015444)。（应用上报去电时，VoipCallAttribute中的voipCallState属性必须为VOIP_CALL_STATE_DIALING。） |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<[ErrorReason](#section15252018876)> | Promise对象，返回错误码类型。 |
**错误码：**
以下错误码的详细介绍请参见 [ArkTS API错误码](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Call Service Kit（通话服务）/ArkTS API/call-error-code.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
...
应用上报去电。需设置通话详细信息，见 [VoipCallAttribute](#section4221135015444) 。使用Promise异步回调。
**系统能力** ：SystemCapability.Telephony.VoipCallManager
**起始版本：** 5.0.0(12)
**参数：**
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| voipCallAttribute | [VoipCallAttribute](#section4221135015444) | 是 | 应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见[VoipCallAttribute](#section4221135015444)。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<[ErrorReason](#section15252018876)> | Promise对象，返回错误码类型。 |
...
| callAudioEvent | [CallAudioEvent](#section312217120409) | 是 | VoIP开/关静音、扬声器事件，详情请参见[CallAudioEvent](#section312217120409)。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
...
该接口不能改变通话类型，例如，语音通话不能升级为视频通话，视频通话也不能降级为语音通话，如需上述升降级操作，请调用 [voipCall.reportCallStateChange](#section5360122718341) 。
**系统能力** ：SystemCapability.Telephony.VoipCallManager
**起始版本：** 4.1.0(11)
**参数：**
| 参数名 | 类型 | 必填 | 说明 |
...
| callState | [VoipCallState](#section24523814719) | 是 | 通话状态，参考[VoipCallState](#section24523814719)。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象。无返回结果的Promise对象。 |
...
| callId | string | 是 | 呼叫ID。 |
| callState | [VoipCallState](#section24523814719) | 是 | 通话状态，参考[VoipCallState](#section24523814719)。 |
| callType | [VoipCallType](#section0933646875) | 是 | 通话类型，参考[VoipCallType](#section0933646875)。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象。无返回结果的Promise对象。 |
...
| --- | --- | --- | --- |
| callId | string | 是 | 应用内通话唯一ID。 |
| voipCallFailureCause | [VoipCallFailureCause](#section157617591167) | 是 | 来电消息建立失败原因。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象。无返回结果的Promise对象。 |
