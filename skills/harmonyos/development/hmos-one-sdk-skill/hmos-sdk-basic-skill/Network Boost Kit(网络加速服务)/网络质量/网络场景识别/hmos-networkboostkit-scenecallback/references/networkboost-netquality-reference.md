...
| callback | Callback<Array<[NetworkQos](#section5777123521219)>> | 是 | 回调函数，返回网络Qos变化的详细信息。 |
**错误码** ：
...
| callback | Callback<Array<[NetworkQos](#section5777123521219)>> | 否 | 需要取消注册的回调函数，需与订阅时传入的回调函数是同一个。若无此参数，则取消注册所有的回调函数。 |
**错误码：**
涉及错误码均为通用错误码， [通用错误码详细描述查看](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal) 。
| 错误码 | 说明 |
| --- | --- |
...
| callback | Callback<Array<[NetworkScene](#section19886656145619)>> | 是 | 回调函数，返回网络场景变化的详细信息。 |
**错误码：**
涉及错误码均为通用错误码， [通用错误码详细描述查看](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal) 。
| 错误码 | 说明 |
| --- | --- |
...
| --- | --- | --- | --- |
| type | string | 是 | 固定填写"netSceneChange"字符串，表示网络场景变化事件。 |
| callback | Callback<Array<[NetworkScene](#section19886656145619)>> | 否 | 需要取消注册的回调函数，需与订阅时传入的回调函数是同一个。若无此参数，则取消注册所有的回调函数。 |
**错误码：**
涉及错误码均为通用错误码， [通用错误码详细描述查看](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal) 。
| 错误码 | 说明 |
| --- | --- |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| appQoe | [AppQoe](#section25523227370) | 是 | 表示应用需要反馈的传输体验信息。 |
**错误码：**
涉及错误码均为通用错误码， [通用错误码详细描述查看](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/errorcode-universal) 。
| 错误码 | 说明 |
| --- | --- |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| serviceType | [ServiceType](#section18954312204111) | 否 | 否 | 应用的业务类型。 |
| qoeType | [QoeType](#section819973417438) | 否 | 否 | 应用体验类型。 |
#### NetworkQos
网络质量回调信息。
**系统能力：** SystemCapability.Communication.NetworkBoost.Core
**起始版本：** 5.0.0(12)
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| pathType | [PathType](#section45141941132511) | 否 | 否 | 表明相应的数据路径上的网络质量信息。 |
| linkUpBandwidth | [RateBps](#section786722592713) | 否 | 否 | 上行带宽信息，单位为bps。 |
| linkDownBandwidth | [RateBps](#section786722592713) | 否 | 否 | 下行带宽信息，单位为bps。 |
| linkUpRate | [RateBps](#section786722592713) | 否 | 否 | 上行速率，单位为bps。 |
| linkDownRate | [RateBps](#section786722592713) | 否 | 否 | 下行速率，单位为bps。 |
...
| [BadQoeCause](#section327672810449) | 表示体验差类型。 |
#### BadQoeCause
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| pathType | [PathType](#section45141941132511) | 否 | 否 | 表明相应的数据路径上的网络场景信息。 |
| scene | [Scene](#section18341052913) | 否 | 否 | 场景状态。 |
| recommendedAction | [RecommendedAction](#section1272138831) | 否 | 否 | 建议的数传策略。 |
| weakSignalPrediction | [WeakSignalPrediction](#section1593261915415) | 否 | 是 | 弱信号预测相关信息。 |
...
| [DataSpeedSimpleAction](#section23982281626) | 应用发包策略的简单建议。 |