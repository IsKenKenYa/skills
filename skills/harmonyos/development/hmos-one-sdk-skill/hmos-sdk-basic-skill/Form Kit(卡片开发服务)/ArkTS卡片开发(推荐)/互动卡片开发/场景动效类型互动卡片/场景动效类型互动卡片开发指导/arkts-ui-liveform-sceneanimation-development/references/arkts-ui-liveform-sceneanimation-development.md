...
| 接口名 | 描述 |
| --- | --- |
| [onLiveFormCreate(liveFormInfo: LiveFormInfo, session: UIExtensionContentSession): void](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-liveformextensionability.md) | 互动卡片界面对象创建的回调函数。 |
| [onLiveFormDestroy(liveFormInfo: LiveFormInfo): void](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-liveformextensionability.md) | 互动卡片界面对象销毁、资源清理的回调函数。 |
| [LiveFormExtensionContext](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/application/js-apis-application-liveformextensioncontext.md) | LiveFormExtensionAbility的上下文，继承自ExtensionContext。 |
| [startAbilityByLiveForm(want: Want): Promise<void>](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/application/js-apis-application-liveformextensioncontext.md) | 拉起互动卡片提供方（应用）的页面。 |
| [formProvider.requestOverflow(formId: string, overflowInfo: formInfo.OverflowInfo): Promise<void>](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) | 卡片提供方发起互动卡片动效请求。 |
| [formProvider.cancelOverflow(formId: string): Promise<void>](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) | 卡片提供方发起取消互动卡片动效请求。 |
| [formProvider.getFormRect(formId: string): Promise<formInfo.Rect>](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) | 卡片提供方查询卡片位置、尺寸。 |
#### 开发流程
#### 卡片激活态UI开发
1.
创建互动卡片
通过 [LiveFormExtensionAbility](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-liveformextensionability.md) 创建互动卡片，创建时加载互动卡片页面。
```
...
在module.json5配置文件中 [extensionAbilities标签](D:/code/APIDevice/output/md_output/harmonyos-guides/基础入门/开发基础知识/应用配置文件/module.json5配置文件/module-configuration-file.md) 下配置LiveFormExtensionAbility。
```
...
1.
非激活态卡片页面实现
非激活态卡片页面开发同普通卡片开发流程完全一致，在widgetCard.ets中完成。widgetCard.ets文件在卡片创建时自动生成，卡片创建流程可以参考 [创建ArkTS卡片](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/Form Kit（卡片开发服务）/ArkTS卡片开发（推荐）/创建ArkTS卡片/arkts-ui-widget-creation.md) 。在非激活态卡片页面实现点击卡片时，发起卡片动效请求。
```
...
1.
触发互动卡片动效
互动卡片通过调用 [formProvider.requestOverflow](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) 接口触发动效，调用时需要明确：（1）动效申请范围。（2）动效持续时间。（3）是否使用系统提供的默认切换动效。具体可参考 [formInfo.OverflowInfo](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-forminfo.md) 。其中，互动卡片可以通过调用 [formProvider.getFormRect](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) 接口获取卡片尺寸和在窗口内的位置信息。卡片提供方以此计算动效申请范围，单位为vp。计算规则具体请参考 [互动卡片请求参数约束](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/Form Kit（卡片开发服务）/ArkTS卡片开发（推荐）/互动卡片开发/场景动效类型互动卡片/场景动效类型互动卡片概述/arkts-ui-liveform-sceneanimation-overview.md) 。
```
...
```
#### 实现效果
以下是按照本文档代码示例开发而成的效果demo，demo执行动效时，点击按钮，将调用 [formProvider.cancelOverflow](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) 接口，打断当前破框动效，卡片切换为非激活态。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8b/v3/krbvFLF3SzSKC3edeod31g/zh-cn_image_0000002659100243.gif?HW-CC-KV=V1&HW-CC-Date=20260701T104720Z&HW-CC-Expire=86400&HW-CC-Sign=FC8653501925720F8D3B55D171F4A2A0FA3A24A42098C0E2AB383D04C62A5AF5)