...
---
# 通过router或call事件刷新卡片内容
使用router事件，点击卡片可拉起对应应用的UIAbility至前台，并刷新卡片。使用call事件，点击卡片可拉起对应应用的UIAbility至后台，并刷新卡片。在卡片页面中可以通过 [postCardAction](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/ArkUI（方舟UI框架）/ArkTS API/UI界面/js-apis-postcardaction.md) 接口触发router事件或者call事件拉起UIAbility，然后由UIAbility刷新卡片内容，下面是这种刷新方式的简单示例。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e1/v3/d1zgSNnbTCyJSu9H1-r15A/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T104717Z&HW-CC-Expire=86400&HW-CC-Sign=E87EF9196ACE88C7B73CB383FEE90D839C1E36B526CAAD4ABC40A2C9DF184DA6)
...
```
-
在UIAbility的onCreate或者onNewWant生命周期中可以通过入参want获取卡片的formID和传递过来的参数信息，然后调用 [updateForm](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) 接口刷新卡片。
```
...
```
-
在UIAbility的onCreate生命周期中监听call事件所需的方法，然后在对应方法中调用 [updateForm](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) 接口刷新卡片。
```
