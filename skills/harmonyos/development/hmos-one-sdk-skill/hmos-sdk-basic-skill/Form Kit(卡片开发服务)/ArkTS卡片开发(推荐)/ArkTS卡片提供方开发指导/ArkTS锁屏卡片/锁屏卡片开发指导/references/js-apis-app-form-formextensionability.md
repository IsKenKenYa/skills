...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/17/v3/4hgp3IldQVaFF4rthTITLg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093142Z&HW-CC-Expire=86400&HW-CC-Sign=00F84A023A832C2667931F4389D18B2B035EACA292A6C79A49F8F45D3D467159)
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| --- | --- | --- | --- | --- |
| context | [FormExtensionContext](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/application/js-apis-inner-application-formextensioncontext.md) | 否 | 否 | FormExtensionAbility的上下文环境，继承自[ExtensionContext](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/js-apis-inner-application-extensioncontext.md)。**元服务API：**从API version 11开始，该接口支持在元服务中使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/js-apis-app-ability-want.md) | 是 | 当前卡片相关的Want类型信息，其中Want中的parameters为自定义取值，取值可以包含[卡片参数枚举](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-forminfo.md)中的一个或多个，如卡片ID、卡片名称、卡片样式等。这些卡片信息必须作为持久数据进行管理，以便后续更新和删除卡片。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| [formBindingData.FormBindingData](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formbindingdata.md) | formBindingData.FormBindingData对象，卡片要显示的数据。 |
**示例：**
```
...
卡片提供方接收携带参数的更新卡片的通知接口。获取最新数据后调用formProvider的 [updateForm](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-formprovider.md) 接口刷新卡片数据。
**模型约束：** 此接口仅可在Stage模型下使用。
**元服务API：** 从API version 11开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.Ability.Form
**参数：**
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| newConfig | [Configuration](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/js-apis-app-ability-configuration.md) | 是 | 表示需要更新的配置信息。 |
**示例：**
```
import { FormExtensionAbility } from '@kit.FormKit';
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| want | [Want](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/js-apis-app-ability-want.md) | 是 | want表示获取卡片状态的描述。描述包括Bundle名称、能力名称、模块名称、卡片名称等。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| [formInfo.FormState](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-forminfo.md) | formInfo.FormState枚举，表示卡片当前的状态。 |
**示例：**
```
...
| newFormLocation | [formInfo.FormLocation](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-forminfo.md) | 是 | 卡片最新位置的枚举值。 |
**示例：**
```
...
| newDimension | [formInfo.FormDimension](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-forminfo.md) | 是 | 卡片尺寸，例如 Dimension_1_2，表示 1 x 2 卡片。 |
| newRect | [formInfo.Rect](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Form Kit（卡片开发服务）/ArkTS API/js-apis-app-form-forminfo.md) | 是 | 卡片位置信息，包括卡片左上角顶点的xy坐标和卡片的宽高。 |
**示例：**
```
import { FormExtensionAbility, formInfo } from '@kit.FormKit';
export default class MyFormExtensionAbility extends FormExtensionAbility {
