...
用户首选项的持久化文件存储在 [preferencesDir](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/Ability Kit（程序框架服务）/应用模型/应用组件/应用上下文Context/application-context-stage.md) 路径下，创建preferences对象前，需要保证preferencesDir路径可读写。持久化文件存储路径中的 [加密等级](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/Stage模型能力的接口/js-apis-app-ability-contextconstant.md) 会影响文件的可读写状态，路径访问限制详见 [应用文件目录与应用文件路径](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/Core File Kit（文件基础服务）/应用文件/应用沙箱目录/app-sandbox-directory.md) 。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/2b/v3/xLZLzD58Qtijb5-k7XnuLg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=6E469ADD93EDE214DFC788C03A6DB27BE139EAA64AE76AFA9679C820C22B61AA)
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| context | Context | 是 | 应用上下文。FA模型的应用Context定义见[Context](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/FA模型能力的接口/app/js-apis-inner-app-context.md)。Stage模型的应用Context定义见[Context](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/接口依赖的元素及定义/application/js-apis-inner-application-context.md)。 |
| name | string | 是 | Preferences实例的名称。 |
| callback | AsyncCallback<[Preferences](#preferences)> | 是 | 回调函数。当获取Preferences实例成功，err为undefined，返回Preferences实例；否则err为错误对象。 |
**错误码：**
以下错误码的详细介绍请参见 [通用错误码](D:/code/APIDevice/output/md_output/harmonyos-references/API参考概述/errorcode-universal.md) 和 [用户首选项错误码](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/ArkData（方舟数据管理）/错误码/errorcode-preferences.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
...
调用该接口后，不建议再使用旧的Preferences实例进行数据操作，否则会导致数据一致性问题，应将Preferences实例置为null，系统会统一回收。
若使用 [GSKV存储模式](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md) ，推荐在进程退出时手动调用一次该接口。此操作会将数据缓存页写入磁盘，可一定程度上减少下一次调用getPreferences接口时的耗时。否则，下一次调用getPreferences接口时底层需要进行数据恢复，数据恢复的耗时取决于未写入磁盘的数据缓存页数量。
**元服务API：** 从API version 11开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.DistributedDataManager.Preferences.Core
**参数：**
| 参数名 | 类型 | 必填 | 说明 |
...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| XML | 0 | 表示[XML存储模式](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md)，这是Preferences的默认存储模式。**特点：**数据XML格式进行存储。对数据的操作发生在内存中，需要调用flush接口进行落盘。 |
| GSKV | 1 | 表示[GSKV存储模式](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md)。**特点：**数据以GSKV数据库模式进行存储。对数据的操作实时落盘，无需调用flush接口对数据进行落盘。 |
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/e6/v3/NtXiwa6vSe6oTQIj0XpKHA/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=0D93FE95AF43BDC910FCC4479D775790AB3F77B7E9B139B8767A03C6EC089B00)
...
| dataGroupId | string|null|undefined | 否 | 是 | 应用组ID，需要向应用市场获取，详见[dataGroupId申请流程](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/IME Kit（输入法开发服务）/输入法安全模式介绍/ime-kit-security.md)。基于dataGroupId的数据共享支持两种场景：1.同一应用的不同进程间共享，只支持三方应用中输入法和输入法的扩展场景使用；2.不同应用间的数据共享，只支持系统应用使用。为可选参数。指定在此dataGroupId对应的沙箱路径下创建Preferences实例。当此参数不填时，默认在本应用沙箱目录下创建Preferences实例。**模型约束：**此属性仅在Stage模型下可用。**元服务API：**从API version 11开始，该参数支持在元服务中使用。 |
...
**错误码：**
以下错误码的详细介绍请参见 [用户首选项错误码](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/ArkData（方舟数据管理）/错误码/errorcode-preferences.md) 。
| 错误码ID | 错误信息 |
| --- | --- |
| 15500000 | Inner error. |
**示例：**
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/60/v3/Zg6q_toNTpaBA7kvP_6Xbw/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=642DCFE2E624E6C88CFB0350C1D84C76A585737E20B33C0E921B7B7D97D73F23)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/9e/v3/IyO7K0R7S9q-Kbfb-M4C_w/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=F32E6C447904E71CD5FC847531D6C6D5AE36B897DB8EA688DADD8C894697D7D1)
当value中包含非UTF-8格式的字符串时，请使用Uint8Array类型存储，否则会造成持久化文件出现格式错误造成文件损坏。
当对应的键已经存在时，put()方法会覆盖其值。可以使用hasSync()方法检查是否存在对应键值对。
**元服务API：** 从API version 11开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.DistributedDataManager.Preferences.Core
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/91/v3/w2dS_st1SeeocTBQyDxHrQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=FCB590BFF6DF7727A7BF0C0C619B353DD39D7302548BF486EB1162C8003D635D)
当value中包含非UTF-8格式的字符串时，请使用Uint8Array类型存储，否则会造成持久化文件出现格式错误造成文件损坏。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/b1/v3/n8QcpXs7T8GHGEEVl6OzGg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=BF3C714D270A7D3532B2B010C7EB2EFC9CD1D665A5AFCC12197D42232903D37A)
当数据未修改或修改后的数据与缓存数据一致时，不会刷新持久化文件。
只在XML存储模式下使用，在GSKV存储模式下无需调用，因为当选择该模式时首选项对数据的操作会实时落盘。Preferences存储模式可见 [存储模式说明](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md) 。
**元服务API：** 从API version 11开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.DistributedDataManager.Preferences.Core
**参数：**
| 参数名 | 类型 | 必填 | 说明 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8c/v3/X_FH18M_RY2uojlUXDdTGg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=3DF2A523A779E1EBF1DC05DEC2F860F19334D0C7B57C6FF753D483539CFB9195)
当数据未修改或修改后的数据与缓存数据一致时，不会刷新持久化文件。
只在XML存储模式下使用，在GSKV存储模式下无需调用，因为当选择该模式时首选项对数据的操作会实时落盘。Preferences存储模式可见 [存储模式说明](D:/code/APIDevice/output/md_output/harmonyos-guides/应用框架/ArkData（方舟数据管理）/应用数据持久化/通过用户首选项实现数据持久化 (ArkTS)/data-persistence-by-preferences.md) 。
**元服务API：** 从API version 11开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.DistributedDataManager.Preferences.Core
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5/v3/bpvtI5EGQ9OKPs3WPKGNig/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=6DA6B1C701E1F606E85F29CB2917086E3EF026FCB5CBB8F7198B9F1A31652735)
当数据未修改或修改后的数据与缓存数据一致时，不会刷新持久化文件。
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/87/v3/jl_A2n-xQweJ-g8vNmRaFQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=1C97C3395CE9D963E45C168898AA0A6653411BC340E4576F96229742592E30DC)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/df/v3/7jIPGMbFQ96aE_PiGa6FFQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=E93612CCB9BFDB5F0BDD0D01F378197D0A63CADDE84F23A1FF9D8B552C5238A7)
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/ca/v3/KBJecrggTdyZqC5aAXMNOQ/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T092740Z&HW-CC-Expire=86400&HW-CC-Sign=2FA6FB49549A381D2B63CB7A4D1A52019511FE903EB60439DC5F2225ADD1D6FD)
当调用 [removePreferencesFromCache](#preferencesremovepreferencesfromcache) 或 [deletePreferences](#preferencesdeletepreferences) 后，订阅的数据变更会主动取消订阅，在重新 [getPreferences](#preferencesgetpreferences) 后需要重新订阅数据变更。
