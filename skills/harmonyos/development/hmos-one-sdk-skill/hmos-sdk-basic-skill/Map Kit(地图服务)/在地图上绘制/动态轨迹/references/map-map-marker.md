...
```
#### Marker
标记，继承 [BaseOverlay](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map（地图显示功能）/map-map-baseoverlay.md) 。在调用map. [MapComponentController](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map（地图显示功能）/map-map-mapcomponentcontroller.md) 类的 [addMarker](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map（地图显示功能）/map-map-mapcomponentcontroller.md) 方法时会返回该类型的实例。
...
| 类型 | 说明 |
| --- | --- |
| [mapCommon.LatLng](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map-common.md) | 标记的位置。 |
**示例：**
```typescript
...
| 类型 | 说明 |
| --- | --- |
| boolean | 标记是否可以点击。true：可以false：不可以 |
**示例：**
```typescript
...
| 类型 | 说明 |
| --- | --- |
| boolean | 标记是否可以通过长按来拖拽。true：可以false：不可以 |
**示例：**
```typescript
...
| 类型 | 说明 |
| --- | --- |
| boolean | 标记是否平贴地图。true：平贴地图false：面对相机 |
**示例：**
```typescript
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| alpha | number | 是 | 标记的透明度，取值范围：[0, 1]，默认值为1，异常值不处理。0：完全透明1：完全不透明 |
**示例：**
```typescript
marker.setAlpha(0.5);
```
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| clickable | boolean | 是 | 设置标记是否可以点击，默认值为false。true：可以false：不可以 |
**示例：**
```typescript
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| draggable | boolean | 是 | 是否可以长按拖拽，默认值为false。true：可以false：不可以 |
**示例：**
```typescript
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| flat | boolean | 是 | 是否平贴地图，默认值为false。true：平贴地图false：面对相机 |
**示例：**
```typescript
marker.setFlat(true);
```
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| icon | string |[image.PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)|[Resource](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/ArkUI（方舟UI框架）/ArkTS组件/公共定义/ts-types.md) | 是 | 标记的图标。图片格式支持jpg、jpeg、png、gif、webp、svg。string类型入参支持两种格式：资源相对路径格式：图标存放在resources/rawfile，icon参数传入rawfile文件夹下的相对路径。toDataURL格式（如data:image/png;base64,<图片的Base64字节编码值>）说明：从5.0.0(12)版本开始，icon属性支持[Resource](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/ArkUI（方舟UI框架）/ArkTS组件/公共定义/ts-types.md)和[image.PixelMap](D:/code/APIDevice/output/md_output/harmonyos-references/媒体/Image Kit（图片处理服务）/ArkTS API/@ohos.multimedia.image (图片处理)/arkts-apis-image-pixelmap.md)类型。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/96/v3/6XEsDogiQGi4Xob15crCnA/zh-cn_image_0000002485159994.png?HW-CC-KV=V1&HW-CC-Date=20260401T093826Z&HW-CC-Expire=86400&HW-CC-Sign=D9609B17EAF41651DCE23B3058DF9B5D32AE6033A466905CD7A70BBABBF95342)
**模型约束：** 此接口仅可在Stage模型下使用。
**元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.Map.Core
**起始版本：** 4.1.0(11)
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| latLng | [mapCommon.LatLng](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map-common.md) | 是 | 标记的位置坐标。 |
**示例：**
```typescript
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| visible | boolean | 是 | 设置信息窗是否可见。true：可见false：不可见 |
**示例：**
```typescript
...
| **类型** | **说明** |
| --- | --- |
| boolean | 信息窗口是否可见。true：可见false：不可见 |
**示例：**
```typescript
...
设置标记的动画，不支持 [FontSizeAnimation](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map（地图显示功能）/map-map-fontsizeanimation.md) 。
**模型约束：** 此接口仅可在Stage模型下使用。
**元服务API：** 从版本4.1.0(11)开始，该接口支持在元服务中使用。
**系统能力：** SystemCapability.Map.Core
**起始版本：** 4.1.0(11)
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| animation | [Animation](D:/code/APIDevice/output/md_output/harmonyos-references/应用服务/Map Kit（地图服务）/ArkTS API/map（地图显示功能）/map-map-animation.md) | 是 | 动画。 |
**示例：**
```typescript
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| visible | boolean | 是 | 标记是否显示文本。true：显示false：不显示 |
**示例：**
```typescript
...
| 类型 | 说明 |
| --- | --- |
| boolean | 标记文本是否可见。true：可见false：不可见 |
**示例：**
```typescript
...
| 参数名 | **类型** | 必填 | **说明** |
| --- | --- | --- | --- |
| iconBuilder | [CustomBuilder](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/ArkUI（方舟UI框架）/ArkTS组件/公共定义/ts-types.md) | 是 | 自定义组件。 |
**返回值：**
| 类型 | 说明 |
| --- | --- |
| Promise<void> | Promise对象。无返回结果的Promise对象。 |
