...
# 订阅主线程超时事件（C/C++）
#### 简介
本文介绍如何使用HiAppEvent提供的C/C++接口订阅主线程超时事件。接口的详细使用说明（参数限制、取值范围等）请参考 [hiappevent.h](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md) 。
#### 接口说明
| 接口名 | 描述 |
| --- | --- |
...
```
9.
点击DevEco Studio界面中的运行按钮，运行应用工程，可以快速点击2~3次timeOut350按钮，以触发主线程超时事件。
#### 验证观察者是否订阅到主线程超时事件
1.
...
```
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/65/v3/tL-Uk5c4SBSwMMIPH6AR6g/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T104942Z&HW-CC-Expire=86400&HW-CC-Sign=533D6A084ED9A90608C1391F1FD6CD7A47CA1B42B7B8DC2552A0B39AA1E086EE)
主线程超时事件具体规格可参考： [主线程超时事件默认时间规格](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/故障检测/任务超时检测/apptask-timeout-guidelines.md) 和 [主线程超时事件日志规格](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/故障检测/任务超时检测/apptask-timeout-guidelines.md) 。
#### 移除并销毁事件观察者
1.
移除事件观察者：
```
