...
#undef LOG_TAG
#define LOG_TAG "CrashEventReceiver"
    ...
    for (uint32_t i = 0; i < groupLen; ++i) {
        for (uint32_t j = 0; j < appEventGroups[i].infoLen; ++j) {
            ...
            if (reader.parse(appEventInfo.params, params)) {
                ...
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.release_type=%{public}s", params["release_type"].asString().c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.cpu_abi=%{public}s", params["cpu_abi"].asString().c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.app_running_unique_id=%{public}s", params["app_running_unique_id"].asString().c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.bundle_version=%{public}s", params["bundle_version"].asString().c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.bundle_name=%{public}s", params["bundle_name"].asString().c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.pid=%{public}d", params["pid"].asInt());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.uid=%{public}d", params["uid"].asInt());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.uuid=%{public}s", params["uuid"].asString().c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.exception=%{public}s", writer.write(params["exception"]).c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.hilog.size=%{public}d", params["hilog"].size());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.process_life_time=%{public}d", params["process_life_time"].asInt());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.memory=%{public}s", writer.write(params["memory"]).c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.external_log=%{public}s", writer.write(params["external_log"]).c_str());
                OH_LOG_INFO(LOG_APP, "HiAppEvent eventInfo.params.log_over_limit=%{public}d", params["log_over_limit"].asBool());
            }
        }
    }
}
...
static napi_value RegisterWatcherCrashEvent(napi_env env, napi_callback_info info)
{
    ...
    int ret = OH_HiAppEvent_SetAppEventFilter(systemEventWatcherR, DOMAIN_OS, 0, names, 1);
    if (ret != 0) {
    ...
    if (config != nullptr) {
        ...
        ret = OH_HiAppEvent_SetEventConfig(EVENT_APP_CRASH, config);
...
static napi_value RemoveWatcherCrash(napi_env env, napi_callback_info info)
{
    if (systemEventWatcherR != nullptr) {
        ...
        OH_LOG_INFO(LOG_APP, "Watcher removed successfully");
    }
    return nullptr;
}
