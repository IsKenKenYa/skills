...
# 订阅应用冻屏告警事件（ArkTS）
#### 简介
本文介绍如何使用HiAppEvent提供的ArkTS接口订阅应用冻屏告警事件。接口的详细使用说明（参数限制、取值范围等）请参考 [@ohos.hiviewdfx.hiAppEvent](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/js-apis-hiviewdfx-hiappevent.md) 。
#### 接口说明
| 接口名 | 描述 |
| --- | --- |
...
```
4.
点击DevEco Studio界面中的运行按钮，运行应用工程，然后在应用界面中点击按钮“appFreezeWarning”，触发一次应用冻屏告警事件。
#### 验证观察者是否订阅到应用冻屏告警事件
