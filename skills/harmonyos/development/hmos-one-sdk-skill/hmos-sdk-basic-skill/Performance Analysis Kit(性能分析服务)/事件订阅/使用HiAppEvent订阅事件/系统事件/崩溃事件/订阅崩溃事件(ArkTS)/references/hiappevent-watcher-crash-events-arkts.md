...
# 订阅崩溃事件（ArkTS）
#### 简介
本文介绍如何使用HiAppEvent提供的ArkTS接口订阅应用崩溃事件。接口的详细使用说明（参数限制、取值范围等）请参考 [@ohos.hiviewdfx.hiAppEvent](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/js-apis-hiviewdfx-hiappevent.md) 。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/5e/v3/Pj3voGElRuajbUzNOz3FVg/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T104939Z&HW-CC-Expire=86400&HW-CC-Sign=EA088D007E2094FFE3D0C0ADC28C39E5EC6BFB64A345F079D2EB8A3D307556BD)
...
```
2.
编辑工程中的“entry > src > main > ets > entryability > EntryAbility.ets”文件，在onCreate函数中设置事件的 [崩溃事件自定义参数](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/崩溃事件/崩溃事件介绍/hiappevent-watcher-crash-events.md) 和崩溃日志 [自定义规格设置](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/崩溃事件/崩溃事件介绍/hiappevent-watcher-crash-events.md) ，示例代码如下：
```
// 构建崩溃事件的自定义参数
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/4d/v3/9nKtxCUqSE2OdqgLHjCF5w/note_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260701T104939Z&HW-CC-Expire=86400&HW-CC-Sign=5413FECBF9333CC174F8AF9173E0C2F3228F5938188AA0C1D6B8D0536F102A23)
...
若应用无法启动或长时间未启动，开发者可以参考 [使用FaultLogExtensionAbility订阅事件](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用FaultLogExtensionAbility订阅事件/fault-log-extension-app-events-arkts.md) 回调重写的函数，进行延迟上报。
**应用主动捕获崩溃异常场景**
若应用主动捕获崩溃异常，崩溃事件将在 **应用退出前** 回调，例如以下两种情况：
1.
异常处理中未主动退出，应用崩溃后将不会退出。
采用 [errorManager.on](D:/code/APIDevice/output/md_output/harmonyos-references/应用框架/Ability Kit（程序框架服务）/ArkTS API/通用能力的接口(推荐)/js-apis-app-ability-errormanager.md) 方法捕获异常会导致JsError类型的崩溃事件在应用退出前回调。若应用主动注册 [崩溃信号](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/故障检测/崩溃检测/Cpp Crash（进程崩溃）检测/cppcrash-guidelines.md) 处理函数但未主动退出，会导致NativeCrash类型的崩溃事件在应用退出前回调。
2.
异常处理耗时过长，导致应用退出延迟。
在开发调试阶段，HiAppEvent上报事件完成回调后，可以在DevEco Studio的HiLog窗口查看JsError类型崩溃事件内容。NativeCrash类型崩溃事件内容略有不同，具体参见 [崩溃事件字段说明](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/崩溃事件/崩溃事件介绍/hiappevent-watcher-crash-events.md) 。JsError类型崩溃事件内容样例如下：
```
...
[@ohos.faultLogger (故障日志获取)](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/已停止维护的接口/js-apis-faultlogger.md) 接口从API version 18开始废弃使用, 不再维护。后续版本推荐使用 [@ohos.hiviewdfx.hiAppEvent](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/js-apis-hiviewdfx-hiappevent.md) 订阅崩溃事件。该章节指导开发者从Faultlogger接口迁移至hiAppEvent接口，来订阅崩溃事件。
在Faultlogger的 [FaultType](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/已停止维护的接口/js-apis-faultlogger.md) 里定义的CPP_CRASH和JS_CRASH都属于崩溃故障类型。
在hiAppEvent的hiAppEvent.addWatcher接口中设置事件名称为hiAppEvent.event.APP_CRASH、事件领域为hiAppEvent.domain.OS，可以订阅崩溃事件。
通过 [hiAppEvent.AppEventInfo.params](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/崩溃事件/崩溃事件介绍/hiappevent-watcher-crash-events.md) 中的crash_type字段可以区分具体是哪种崩溃事件。
两者对应关系如下：
| Faultlogger.FaultType | hiAppEvent.AppEventInfo.params.crash_type |
| --- | --- |
| CPP_CRASH | NativeCrash |
| JS_CRASH | JsError |
[FaultLogInfo](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/已停止维护的接口/js-apis-faultlogger.md) 与 [hiAppEvent.AppEventInfo.params](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/崩溃事件/崩溃事件介绍/hiappevent-watcher-crash-events.md) 的对应关系如下：
...
[FaultLogger.query(使用callback回调)](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/已停止维护的接口/js-apis-faultlogger.md) 和 [FaultLogger.query(使用Promise回调)](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/已停止维护的接口/js-apis-faultlogger.md) 都可以使用 [hiAppEvent.addWatcher](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/ArkTS API/js-apis-hiviewdfx-hiappevent.md) 实现相同功能。
