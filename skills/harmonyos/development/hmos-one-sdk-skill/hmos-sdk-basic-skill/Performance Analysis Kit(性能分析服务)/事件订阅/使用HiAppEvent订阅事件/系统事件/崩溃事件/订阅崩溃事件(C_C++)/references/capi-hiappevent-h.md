...
**起始版本：** 8
**相关模块：**  [HiAppEvent](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/模块/capi-hiappevent.md)
#### 汇总
#### 结构体
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
| [HiAppEvent_AppEventInfo](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-appeventinfo.md) | HiAppEvent_AppEventInfo | 单个事件信息，包含事件领域、事件名称、事件类型和事件携带的用json格式字符串表示的自定义参数列表。 |
| [HiAppEvent_AppEventGroup](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-appeventgroup.md) | HiAppEvent_AppEventGroup | 一组事件信息，包含事件组的名称，按名称分组的单个事件信息数组，事件数组的长度。 |
| [ParamListNode*](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-paramlistnode8h.md) | ParamList | 事件参数列表节点。 |
| [HiAppEvent_Watcher](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-watcher.md) | HiAppEvent_Watcher | 用于接收app事件的监听器。 |
| [HiAppEvent_Processor](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-processor.md) | HiAppEvent_Processor | 应用事件上报的处理者。 |
| [HiAppEvent_Config](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-config.md) | HiAppEvent_Config | 用于设置系统事件触发条件的配置对象。 |
#### 枚举
| 名称 | typedef关键字 | 描述 |
| --- | --- | --- |
...
| [typedef void (*OH_HiAppEvent_OnTrigger)(int row, int size)](#oh_hiappevent_ontrigger) | OH_HiAppEvent_OnTrigger | 监听器收到事件后，若监听器中未设置OH_HiAppEvent_OnReceive回调，将保存该事件。当保存的事件满足通过OH_HiAppEvent_SetTriggerCondition设定的条件后，将触发该回调。回调结束后，当新保存的事件消息再次满足设定的条件后，将再次进行回调。 |
...
| [ParamList OH_HiAppEvent_CreateParamList(void)](#oh_hiappevent_createparamlist) | - | 创建一个指向参数列表对象的指针。**注意**：创建的指向参数列表对象的指针不再使用后，必须通过[OH_HiAppEvent_DestroyParamList](#oh_hiappevent_destroyparamlist)接口进行销毁。 |
...
| [HiAppEvent_Watcher* OH_HiAppEvent_CreateWatcher(const char* name)](#oh_hiappevent_createwatcher) | - | 创建一个用于监听app事件的监听器。**注意**：创建的监听器不再使用后，必须通过[OH_HiAppEvent_DestroyWatcher](#oh_hiappevent_destroywatcher)接口进行销毁。 |
...
| [int OH_HiAppEvent_SetTriggerCondition(HiAppEvent_Watcher* watcher, int row, int size, int timeOut)](#oh_hiappevent_settriggercondition) | - | 用于设置监听器OH_HiAppEvent_OnTrigger回调的触发条件。分别可以从监视器新接收事件数量、新接收事件大小、onTrigger触发超时时间，设置触发条件。调用方应至少保证从一个方面设置触发条件。 |
...
| [int OH_HiAppEvent_SetWatcherOnTrigger(HiAppEvent_Watcher* watcher, OH_HiAppEvent_OnTrigger onTrigger)](#oh_hiappevent_setwatcherontrigger) | - | 用于设置监听器onTrigger回调的接口。如果未设置OnReceive回调或已将其设置为nullptr，则将保存观察者接收到的应用事件。当保存的应用事件满足onTrigger回调的触发条件时，将调用onTrigger回调。 |
...
| [int OH_HiAppEvent_AddWatcher(HiAppEvent_Watcher* watcher)](#oh_hiappevent_addwatcher) | - | 添加监听器的接口，监听器开始监听系统消息。**注意**：OH_HiAppEvent_AddWatcher接口涉及I/O操作。在对性能敏感的业务场景中，开发者应根据实际需要确定该接口是在主线程还是在子线程中调用。订阅接口OH_HiAppEvent_AddWatcher传入的名称name是唯一的，相同的name，后一次调用会覆盖前一次的订阅。 |
...
| [HiAppEvent_Processor* OH_HiAppEvent_CreateProcessor(const char* name)](#oh_hiappevent_createprocessor) | - | 创建一个用于处理app事件上报的处理者。**注意**：创建的处理者不再使用后，必须通过[OH_HiAppEvent_DestroyProcessor](#oh_hiappevent_destroyprocessor)接口进行销毁。 |
...
| [HiAppEvent_Config* OH_HiAppEvent_CreateConfig(void)](#oh_hiappevent_createconfig) | - | 创建一个指向设置系统事件触发条件的配置对象的指针。**注意**：创建的指向设置系统事件触发条件的配置对象的指针不再使用后，必须通过[OH_HiAppEvent_DestroyConfig](#oh_hiappevent_destroyconfig)接口进行销毁。 |
...
| [int OH_HiAppEvent_SetEventConfig(const char* name, HiAppEvent_Config* config)](#oh_hiappevent_seteventconfig) | - | 事件相关的配置参数设置方法。不同的事件有不同的配置项，目前仅支持以下事件：MAIN_THREAD_JANK（参数配置详见[主线程超时事件检测](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/主线程超时事件/主线程超时事件介绍/hiappevent-watcher-mainthreadjank-events.md)）MAIN_THREAD_JANK_V2（参数配置详见[主线程超时事件检测](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/主线程超时事件/主线程超时事件介绍/hiappevent-watcher-mainthreadjank-events.md)） |
#### 枚举类型说明
...
| HIAPPEVENT_INVALID_PARAM_VALUE_LENGTH = 4 | 参数值长度无效。**起始版本：**18 |
| HIAPPEVENT_PROCESSOR_IS_NULL = -7 | 事件处理者为空。**起始版本：**18 |
| HIAPPEVENT_PROCESSOR_NOT_FOUND = -8 | 事件处理者不存在。**起始版本：**18 |
| HIAPPEVENT_INVALID_PARAM_VALUE = -9 | 参数值无效。 |
| HIAPPEVENT_EVENT_CONFIG_IS_NULL = -10 | 事件配置为空。 |
| HIAPPEVENT_OPERATE_FAILED = -100 | 操作失败。**起始版本：**18 |
| HIAPPEVENT_INVALID_UID = -200 | 无效的用户标识。**起始版本：**18 |
#### EventType
```c
enum EventType
```
...
| [const struct HiAppEvent_AppEventGroup](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-appeventgroup.md)* appEventGroups | 按照不同事件名称分组的事件组数组。 |
...
**描述**
创建一个指向参数列表对象的指针。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/60/v3/3RpO2NfgRF2hBYgcJn_QOA/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093451Z&HW-CC-Expire=86400&HW-CC-Sign=55CFDA20858F535CB90A37989DBDBA41A271E9B3191B6FFB0EC951879CD55CA5)
创建的指向参数列表对象的指针不再使用后，必须通过 [OH_HiAppEvent_DestroyParamList](#oh_hiappevent_destroyparamlist) 接口进行销毁。
**起始版本：** 8
**返回：**
| 类型 | 说明 |
| --- | --- |
| [ParamList](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-paramlistnode8h.md) | 指向参数列表对象的指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ParamList](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-paramlistnode8h.md)list | 参数列表对象指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [ParamList](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-paramlistnode8h.md)list | 需要添加参数的参数列表指针。 |
| const char* name | 需要添加的参数名称。 |
| bool boolean | 需要添加的布尔参数值。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
| [ParamList](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-paramlistnode8h.md) | 添加参数后的参数列表指针。 |
...
| enum EventType type | 事件类型，在[EventType](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)中定义。 |
| [const ParamList](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-paramlistnode8h.md)list | 事件参数列表，每个参数由参数名和参数值组成，其规格定义如下：1、参数名为字符串类型。首字符必须为字母字符或$字符，中间字符必须为数字字符、字母字符或下划线字符，结尾字符必须为数字字符或字母字符，长度非空且不超过32个字符。2、参数值支持字符串、数值、布尔、数组类型。字符串类型参数长度需在8*1024个字符以内，超出会做丢弃处理；数组类型参数中的元素类型只能为字符串、数值、布尔中的一种，且元素个数需在100以内，超出会做丢弃处理。3、参数个数需在32个以内，超出的参数会做丢弃处理。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
...
| 参数项 | 描述 |
| --- | --- |
| const char* name | 配置项名称。名称可填[DISABLE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-cfg-h.md)和[MAX_STORAGE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-cfg-h.md)。 |
| const char* value | 配置项值。如果配置项名称是[DISABLE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-cfg-h.md)，值可以填“true”或者“false”；如果配置项名称是[MAX_STORAGE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-cfg-h.md)，配额值字符串只由数字字符和大小单位字符（单位字符支持[b|k|kb|m|mb|g|gb|t|tb]，不区分大小写）构成。配额值字符串必须以数字开头，后面可以选择不传单位字符（默认使用byte作为单位），或者以单位字符结尾。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
...
**描述**
创建一个用于监听app事件的监听器。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/c3/v3/jpoBskelRniJkIp-DXCXwA/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093451Z&HW-CC-Expire=86400&HW-CC-Sign=DA5564547B460F4771C7B4175FED704ABA3DB787E1A3340B82B6648216648CAE)
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Watcher](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-watcher.md)* | 接口调用成功时返回指向的新建监听器的指针，name参数异常时返回nullptr。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Watcher](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-watcher.md)* watcher | 指向监听器的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |
...
| int timeOut | 单位秒，当输入值大于0，每经过timeout秒，将检查监视器是否存在新接收到的事件，如果存在将触发onTrigger回调。触发onTrigger后，经过timeOut秒后将再次检查是否存在新接收到的事件。当输入值小于等于0，不以超时时间为维度触发onTrigger回调。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
...
| uint8_t eventTypes | 需要监听事件的事件类型。使用按位与方式进行匹配，可支持监听多种类型的事件。第一位为1（数值为1）表示支持监听故障类型的事件；第二位为1（数值为2）表示支持监听统计类型的事件；第三位为1（数值为4）表示支持监听安全类型的事件；第四位为1（数值为8）表示支持监听行为类型的事件。都为1（数值为15）或者都为0（数值为0）表示支持所有类型事件。 |
...
| --- | --- |
| [HiAppEvent_Watcher](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-watcher.md)* watcher | 指向监听器的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |
| [OH_HiAppEvent_OnTrigger](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)onTrigger | 需要设置的回调。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
| int | 0：接口调用成功；-5：watcher入参空指针。 |
...
| --- | --- |
| [HiAppEvent_Watcher](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-watcher.md)* watcher | 指向监听器的指针（即OH_HiAppEvent_CreateWatcher接口返回的指针）。 |
| [OH_HiAppEvent_OnReceive](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)onReceive | 回调函数的函数指针。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
| int | 0：接口调用成功；-5：watcher入参空指针。 |
...
| [OH_HiAppEvent_OnTake](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)onTake | 回调函数指针，事件通过调用该函数返回事件信息。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/8a/v3/bCM_N3JLTaqhc5SzCGkwTw/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093451Z&HW-CC-Expire=86400&HW-CC-Sign=B1416DF6A3A79470CA366E465486E8A362E427DE7382F3010DC299CC0C5B5E40)
...
**描述**
创建一个用于处理app事件上报的处理者。
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/1c/v3/SiuE9PTDTI61NzGr4OAaDQ/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093451Z&HW-CC-Expire=86400&HW-CC-Sign=F0F83AAA7D7E7F7F67E687091EB8B8E81A20A03F71F1712922ACC2CDEC3D118E)
...
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Processor](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-processor.md)* | 接口调用成功时返回指向的新建处理者的指针，name参数异常时返回nullptr。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Processor](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-processor.md)* processor | 指向处理者的指针（即OH_HiAppEvent_CreateProcessor接口返回的指针）。 |
| const char* appId | 处理者的应用ID。 |
| const char* routeInfo | 服务器位置信息，默认为空字符串。传入字符串长度不能超8KB，超过时会被置为默认值。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：接口调用成功；[HIAPPEVENT_PROCESSOR_IS_NULL](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：processor入参为空；[HIAPPEVENT_INVALID_PARAM_VALUE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：参数值无效；[HIAPPEVENT_INVALID_UID](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：用户标识无效；[HIAPPEVENT_INVALID_PARAM_VALUE_LENGTH](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：参数值长度无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：接口调用成功；[HIAPPEVENT_PROCESSOR_IS_NULL](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：processor入参为空；[HIAPPEVENT_INVALID_PARAM_VALUE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：参数值无效；[HIAPPEVENT_INVALID_UID](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：用户标识无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |
...
| --- | --- |
| [HiAppEvent_Processor](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-processor.md)* processor | 指向处理者的指针（即OH_HiAppEvent_CreateProcessor接口返回的指针）。 |
| const char* configName | 数据处理者的配置名称，系统会根据配置名称从系统预置的processor.json配置文件中自动加载对应的配置项。只能包含大小写字母、数字、下划线和$，不能以数字开头，长度非空且不超过256个字符。当前取值仅支持“SDK_OCG”，用于配置“接口故障处理者”可处理的事件名称等信息。 |
**返回：**
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：接口调用成功；[HIAPPEVENT_PROCESSOR_IS_NULL](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：processor入参为空；[HIAPPEVENT_INVALID_PARAM_VALUE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：参数值无效；[HIAPPEVENT_INVALID_UID](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：用户标识无效。[HIAPPEVENT_INVALID_PARAM_VALUE_LENGTH](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：参数值长度无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |
...
| 类型 | 说明 |
| --- | --- |
| int64_t | 调用成功时返回处理者唯一ID，大于0；[HIAPPEVENT_PROCESSOR_IS_NULL](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：processor入参为空；[HIAPPEVENT_INVALID_PARAM_VALUE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：参数值无效；[HIAPPEVENT_OPERATE_FAILED](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：数据处理者名称未找到或注册失败；[HIAPPEVENT_INVALID_UID](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：用户标识无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：接口调用成功；[HIAPPEVENT_PROCESSOR_NOT_FOUND](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：事件处理者不存在；[HIAPPEVENT_OPERATE_FAILED](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：操作失败；[HIAPPEVENT_INVALID_UID](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：用户标识无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |
...
![](https://contentcenter-vali-drcn.dbankcdn.cn/pvt_2/DeveloperAlliance_scene_100_1/d/v3/vgOQkpMPQi-8pfr9zgVvbg/caution_3.0-zh-cn.png?HW-CC-KV=V1&HW-CC-Date=20260401T093451Z&HW-CC-Expire=86400&HW-CC-Sign=85F955B0DB5AB6D223B3531A82205A6FCDB9A6D4592BD791744D2D9F9EB10D18)
创建的指向设置系统事件触发条件的配置对象的指针不再使用后，必须通过 [OH_HiAppEvent_DestroyConfig](#oh_hiappevent_destroyconfig) 接口进行销毁。
**起始版本：** 15
**返回：**
| 类型 | 说明 |
| --- | --- |
| [HiAppEvent_Config](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-config.md)* | 指向设置系统事件触发条件的配置对象的指针。 |
...
| 参数项 | 描述 |
| --- | --- |
| [HiAppEvent_Config](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/结构体/capi-hiappevent-hiappevent-config.md)* config | 指向配置对象的指针（即OH_HiAppEvent_CreateConfig接口返回的指针）。 |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：接口调用成功；[HIAPPEVENT_EVENT_CONFIG_IS_NULL](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：传入的指向配置对象的指针为空；[HIAPPEVENT_INVALID_PARAM_VALUE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：设定的配置项无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |
...
MAIN_THREAD_JANK（参数配置详见 [主线程超时事件检测](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/主线程超时事件/主线程超时事件介绍/hiappevent-watcher-mainthreadjank-events.md) ）
MAIN_THREAD_JANK_V2（参数配置详见 [主线程超时事件检测](D:/code/APIDevice/output/md_output/harmonyos-guides/系统/调测调优/Performance Analysis Kit（性能分析服务）/事件订阅/使用HiAppEvent订阅事件/系统事件/主线程超时事件/主线程超时事件介绍/hiappevent-watcher-mainthreadjank-events.md) ）
**起始版本：** 15
**参数：**
| 参数项 | 描述 |
| --- | --- |
...
| 类型 | 说明 |
| --- | --- |
| int | [HIAPPEVENT_SUCCESS](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：接口调用成功；[HIAPPEVENT_INVALID_PARAM_VALUE](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)：设置的参数无效。具体可参考[HiAppEvent_ErrorCode](D:/code/APIDevice/output/md_output/harmonyos-references/系统/调测调优/Performance Analysis Kit（性能分析服务）/C API/头文件/capi-hiappevent-h.md)。 |