...
5. 新增TestHiCollieTimerNdk函数。编辑"napi_init.cpp"文件，新增TestHiCollieTimerNdk函数，构造任务执行超时事件：
```
...
6. 将TestHiCollieTimerNdk注册为ArkTS接口。编辑"napi_init.cpp"文件，TestHiCollieTimerNdk注册为ArkTS接口：
```
EXTERN_C_START
