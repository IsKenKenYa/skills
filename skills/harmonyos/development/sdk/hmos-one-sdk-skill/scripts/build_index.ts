#!/usr/bin/env node
import { existsSync, lstatSync, mkdirSync, mkdtempSync, openSync, closeSync, readFileSync,
  readdirSync, renameSync, rmdirSync, unlinkSync, writeFileSync, statSync } from 'node:fs';
import { dirname, relative, resolve, sep } from 'node:path';
import { gzipSync } from 'node:zlib';
import { randomUUID } from 'node:crypto';
import { performance } from 'node:perf_hooks';
import { DocumentTokenizer } from './document_tokenizer.ts';
import { readCorpus } from './corpus_source.ts';
import { parseFrontmatter } from './corpus.ts';
import { CJK_WORD, STOPWORDS } from './tokenizer.ts';
import { defaultCorpus, INDEX_DIR, INDEX_FILES, SNAPSHOT_FILE } from './storage.ts';
import { isMain } from './program.ts';
import type { Document, InvertedIndex, IndexMeta, LoadedIndex } from './types.ts';

export const BUILDER_VERSION = 'jieba-ts-dag-hmm-v1';
export const FIELD_WEIGHTS = { title: 5, description: 3, breadcrumb: 2, path: 1, body: 1 } as const;
export function documentFields(path: string, content: string, fallbackTitle = 'SUB_SKILL') {
...
export function buildMemory(source: ReturnType<typeof readCorpus>, tokenizer = new DocumentTokenizer()): LoadedIndex {
  const documents: Document[] = [], inverted: InvertedIndex = Object.create(null);
  ...
  for (const entry of source.documents) {
    ...
    for (const field of Object.keys(FIELD_WEIGHTS) as (keyof typeof FIELD_WEIGHTS)[]) {
      for (const term of tokenizer.tokenize(fields[field])) tf.set(term, (tf.get(term) || 0) + FIELD_WEIGHTS[field]);
    }
    const length = [...tf.values()].reduce((a, b) => a + b, 0);
    if (!length) { skipped++; continue; }
    ...
    for (const [term, count] of tf) {
      ...
      maxTf = Math.max(maxTf, count); postingEntries++;
    }
  }
  if (!documents.length) throw new Error('语料没有可索引 token；原索引未改动');
  const terms: Record<string, number> = Object.create(null), segmentationOnly: string[] = [];
  for (const [term, postings] of Object.entries(inverted)) {
    if (!CJK_WORD.test(term)) continue;
    let sum = 0;
    for (let i = 1; i < postings.length; i += 2) sum += postings[i];
    terms[term] = Math.max(1, tokenizer.frequencies.get(term) || Math.max(1000, Math.trunc(sum)));
  }
  for (const term of STOPWORDS) {
    if (!CJK_WORD.test(term) || Object.hasOwn(terms, term)) continue;
    terms[term] = Math.max(1, tokenizer.frequencies.get(term) || 1); segmentationOnly.push(term);
  }
...
export interface BuildOptions { source?: string; output?: string; force?: boolean; snapshot?: boolean }
export interface BuildResult {
  output: string; meta: IndexMeta; elapsed_ms: number; peak_rss_mib: number;
  core_index_bytes: number; corpus_snapshot_bytes: number;
}
const ALLOWED = new Set<string>([...INDEX_FILES, SNAPSHOT_FILE]);
function validateOutput(output: string): void {
  if (!existsSync(output)) return;
  if (!lstatSync(output).isDirectory() || lstatSync(output).isSymbolicLink()) throw new Error('索引输出必须是普通目录');
  for (const entry of readdirSync(output, { withFileTypes: true })) {
    if (!ALLOWED.has(entry.name) || !entry.isFile()) throw new Error('拒绝覆盖包含非索引文件的目录: ' + entry.name);
  }
}
function removeOwnedDirectory(directory: string): void {
  // Only our validated, generated files; never recursively delete arbitrary paths.
  validateOutput(directory);
  for (const name of readdirSync(directory)) unlinkSync(resolve(directory, name));
  rmdirSync(directory);
}
export function buildIndex(options: BuildOptions = {}): BuildResult {
  ...
  catch (error) { throw new Error('建库锁已存在或无法创建: ' + lock + '；请等待其他构建结束，异常退出后需人工确认并清理锁', { cause: error }); }
  ...
  try {
    ...
    // Read the corpus before replacing anything; failures leave the old index intact.
    ...
    const zip = (name: string, value: unknown) =>
      writeFileSync(resolve(stage!, name), gzipSync(Buffer.from(JSON.stringify(value)), { level: 9 }));
    zip('docs.json.gz', index.documents); zip('inverted.json.gz', index.inverted);
    ...
    if (existsSync(output)) {
      backup = output + '.previous-' + randomUUID(); renameSync(output, backup);
    }
    try { renameSync(stage, output); stage = undefined; }
    catch (error) { if (backup) { renameSync(backup, output); backup = undefined; } throw error; }
    if (backup) { removeOwnedDirectory(backup); backup = undefined; }
    ...
    if (stage && existsSync(stage)) removeOwnedDirectory(stage);
    closeSync(descriptor); unlinkSync(lock);
  }
}
export function checkIndex(sourcePath = defaultCorpus(), output = INDEX_DIR): boolean {
  const source = readCorpus(sourcePath);
  if (!INDEX_FILES.every(name => existsSync(resolve(output, name)))) return false;
...
export function main(args = process.argv.slice(2)): number {
  ...
  try {
    for (let i = 0; i < args.length; i++) {
      ...
      else if (arg === '--snapshot' || arg === '--with-snapshot') options.snapshot = true;
    ...
    // Keep the document-set baseline CLI behavior: a plain invocation is
    // idempotent. A stale or legacy index still requires explicit --force, and
    // the lower-level buildIndex() API also refuses implicit overwrites.
    ...
    if (!options.force && current && (!options.snapshot || snapshotReady)) {
      const meta = JSON.parse(readFileSync(resolve(output, 'meta.json'), 'utf8')) as IndexMeta;
      console.log(`[skip] 索引已是最新（built_at=${String(meta.built_at || 'unknown')}）。使用 --force 强制重建。`);
      return 0;
    }
    console.log(JSON.stringify(buildIndex(options), null, 2)); return 0;
