...
import type { LoadedIndex } from './types.ts';
import { ROOT, INDEX_DIR, INDEX_FILES, defaultCorpus, loadGzipJson } from './storage.ts';
export { ROOT, INDEX_DIR, INDEX_FILES, loadGzipJson } from './storage.ts';

export function readIndex(directory = INDEX_DIR, autoBuild = true): LoadedIndex {
  const lock = resolve(directory) + '.build.lock';
  if (existsSync(lock)) throw new Error('索引正在构建，请稍后重试: ' + lock);
  if (!INDEX_FILES.every(name => existsSync(resolve(directory, name))) && autoBuild) {
    ...
    // A separate process releases the full dictionary/HMM after building.
    // Its stdout is captured so the public search JSON remains unchanged.
    ...
    if (run.error || run.status !== 0) throw new Error('自动建库失败: ' + (run.stderr || run.error?.message));
  }
  const metaFile = resolve(directory, 'meta.json');
  ...
  if (existsSync(lock) || readFileSync(metaFile, 'utf8') !== before) throw new Error('读取过程中索引已更新，请重试');
