...
import { readIndex } from './index_store.ts';
import { clearSnippetCache } from './corpus.ts';
import { JiebaTokenizer } from './tokenizer.ts';
import { buildTokenWeights, decide, roundEvidence, THRESHOLDS, validateMode } from './rejection.ts';
import type { Candidate, Document, LoadedIndex, SearchExplanation, SearchOptions } from './types.ts';

export function normalizePath(value: string): string { return value.replaceAll('\\', '/').toLowerCase().replace(/^[./]+/, ''); }
export function documentKit(doc: Document): string {
  const parts = doc.path.replaceAll('\\', '/').split('/');
  return parts[0] === 'hmos-sdk-basic-skill' ? (parts[1] || doc.category) : doc.category;
}
function resolveCategory(category: string, known: string[]): string {
...
export class JiebaRuntime {
  ...
  readonly tokenizer: JiebaTokenizer;
  private readonly scores: Float64Array;
  private readonly seen: Uint8Array;
  private readonly norms: Float64Array;
  private readonly topSlots: Int32Array;
  private readonly matched = new Float64Array(10);
  private readonly numerators = new Float64Array(10);
  private readonly touched: number[] = [];
  private readonly kits: string[];
  ...
  explain(query: string, options: SearchOptions = {}): SearchExplanation {
    if (typeof query !== 'string') throw new Error('query 必须是字符串');
    const mode = validateMode(options.mode ?? 'default');
    const top = options.top ?? 10;
    if (!Number.isInteger(top) || top < 1) throw new Error('top 必须是正整数');
    if (options.category !== undefined && typeof options.category !== 'string') throw new Error('category 必须是字符串');
    ...
    this.scores.fill(0); this.seen.fill(0); this.touched.length = 0;
    for (const [term, query_tf] of counts) {
      ...
      for (let offset = 0; offset < postings.length; offset += 2) {
        ...
        if (!this.seen[doc_id]) { this.seen[doc_id] = 1; this.touched.push(doc_id); }
      }
    }
    ...
    // Refusal features are only consumed by raw Top10. A second postings pass
    // avoids retaining per-term evidence objects for every recalled document.
    this.topSlots.fill(-1); this.matched.fill(0); this.numerators.fill(0);
    ...
    const candidates: Candidate[] = ranked.map((doc_id, offset) => {
...
let singleton: JiebaRuntime | undefined;
export function getRuntime(): JiebaRuntime { return singleton ??= new JiebaRuntime(); }
/** Call after an explicit rebuild; existing runtime instances keep their snapshot. */
export function reloadRuntime(): JiebaRuntime {
  const next = new JiebaRuntime();
  singleton = next; clearSnippetCache(); return next;
}
