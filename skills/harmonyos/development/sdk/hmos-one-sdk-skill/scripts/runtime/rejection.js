export const MODES = ['default', 'conservative', 'aggressive'];
...
export function validateMode(value) {
    if (!MODES.includes(value))
        throw new Error('mode 必须是 default、conservative 或 aggressive');
    return value;
}
export function compact(term) {
    return Array.from(term.normalize('NFKC').toLowerCase()).filter(ch => /[\p{L}\p{N}\u3400-\ufaff]/u.test(ch)).join('');
}
export function buildTokenWeights(terms) {
...
export function roundEvidence(value) {
    if (!Number.isFinite(value))
        throw new Error('non-finite BM25 contribution');
    if (value === 0)
        return 0;
    ...
    if (2n * remainder > denominator || (2n * remainder === denominator && rounded % 2n === 1n))
        rounded++;
    return Math.sign(value) * Number(rounded) / 1e6;
}
export function computeFeatures(weights, matches) {
...
export function decide(features, mode = 'default') {
    const thresholds = THRESHOLDS[validateMode(mode)];
    const failed_gates = [];
    if (!(features.weighted_bm25 > thresholds.bm25))
        failed_gates.push('bm25');
    if (!(features.weighted_query_coverage > thresholds.coverage))
        failed_gates.push('coverage');
