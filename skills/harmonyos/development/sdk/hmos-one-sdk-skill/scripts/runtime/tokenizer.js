...
function splitCamel(token) {
    ...
    if (whole && !pieces.includes(whole))
        pieces.push(whole);
    return pieces;
}
export function expandSpecialTerm(surface) {
...
export class JiebaTokenizer {
    ...
    maxLength;
    logTotal;
    constructor(inverted, lexicon) {
        this.vocabulary = inverted;
        this.frequencies = lexicon.terms;
        let maxLength = 1;
        for (const term in inverted)
            if (Object.hasOwn(inverted, term) && CJK_WORD.test(term))
    ...
    segment(span) {
        ...
        for (let start = 0; start < size;) {
            ...
            if (candidate && Object.hasOwn(this.vocabulary, candidate))
    ...
    tokenize(input) {
        ...
        const tokens = [];
        const nonLatin = (part) => {
            for (const match of part.matchAll(CJK_SPANS))
                tokens.push(...this.segment(match[0]));
        };
        let cursor = 0;
