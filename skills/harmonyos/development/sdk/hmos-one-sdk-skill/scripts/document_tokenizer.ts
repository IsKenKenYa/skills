/**
 * Accurate-mode DAG + HMM port of Jieba 0.42.1 (MIT, see THIRD_PARTY_NOTICES.md).
 * Build-time only: retrieval must never import this module or load these assets.
 */
import { readFileSync } from 'node:fs';
import { gunzipSync } from 'node:zlib';
import { resolve } from 'node:path';
import { JIEBA_ASSET_DIR, loadGzipJson } from './storage.ts';
import { LATIN, STOPWORDS, expandSpecialTerm } from './tokenizer.ts';

const MIN = -3.14e100;
const STATES = ['B', 'M', 'E', 'S'] as const;
type State = typeof STATES[number];
type HMM = { start: Record<State, number>; trans: Record<State, Partial<Record<State, number>>>;
  emit: Record<State, Record<string, number>> };
const PREVIOUS: Record<State, State[]> = { B: ['E', 'S'], M: ['M', 'B'], S: ['S', 'E'], E: ['B', 'M'] };
const HAN_BLOCK = /([\u4e00-\u9fd5a-zA-Z0-9+#&._%\-]+)/u;
const IS_HAN_BLOCK = /^[\u4e00-\u9fd5a-zA-Z0-9+#&._%\-]+$/u;
const HMM_HAN = /([\u4e00-\u9fd5]+)/u;
const HMM_IS_HAN = /^[\u4e00-\u9fd5]+$/u;
const CJK = /[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]/u;
// Python str.isspace includes four information separators not in JS \s.
const SPACE = /[\p{White_Space}\u001c-\u001f]/gu;

export class DocumentTokenizer {
  readonly frequencies = new Map<string, number>();
  readonly totalFrequency: number;
  private readonly prefixes = new Set<string>();
  private readonly model: HMM;
  private readonly folds: Record<string, string>;
  ...
  constructor(assetDirectory = JIEBA_ASSET_DIR) {
    ...
    for (const line of gunzipSync(readFileSync(resolve(assetDirectory, 'dict.txt.gz'))).toString('utf8').split('\n')) {
      ...
      if (!word || !Number.isInteger(freq) || freq < 0) throw new Error('无效 Jieba 词典行');
      this.frequencies.set(word, freq); total += freq;
      for (let end = 1; end <= word.length; end++) this.prefixes.add(word.slice(0, end));
    }
    if (!total) throw new Error('Jieba 词典为空');
    this.totalFrequency = total; this.logTotal = Math.log(total);
  }
  private hmmChinese(sentence: string): string[] {
    ...
    for (let i = 1; i < size; i++) {
      ...
      for (const [s, state] of STATES.entries()) {
        ...
        let best = -Infinity, previous: State = 'B';
        for (const prev of PREVIOUS[state]) {
          const score = scores[STATES.indexOf(prev)] + (this.model.trans[prev][state] ?? MIN) + emit;
          // Python max((score, state)) resolves equal scores lexicographically.
          if (score > best || (score === best && prev > previous)) { best = score; previous = prev; }
        }
        next[s] = best; back[i * 4 + s] = STATES.indexOf(previous);
      }
      scores = next;
    }
    let state = scores[2] > scores[3] ? 2 : 3;
    const path = new Uint8Array(size); path[size - 1] = state;
    for (let i = size - 1; i > 0; i--) { state = back[i * 4 + state]; path[i - 1] = state; }
    const result: string[] = [];
    let begin = 0, next = 0;
    for (let i = 0; i < size; i++) {
      if (path[i] === 0) begin = i;
      else if (path[i] === 2) { result.push(chars.slice(begin, i + 1).join('')); next = i + 1; }
      else if (path[i] === 3) { result.push(chars[i]); next = i + 1; }
    }
    if (next < size) result.push(chars.slice(next).join(''));
    return result;
  }
  private hmm(sentence: string): string[] {
  ...
  private dag(sentence: string): string[] {
    ...
    const flush = () => {
      ...
      else if (buffer) result.push(...(this.frequencies.get(buffer) ? Array.from(buffer) : this.hmm(buffer)));
      buffer = '';
    };
    for (let start = 0; start < size;) {
      const end = ends[start], word = sentence.slice(start, end);
      if (word.length === 1) buffer += word;
      else { flush(); result.push(word); }
      start = end;
    }
    flush(); return result;
  }
  /** Accurate Jieba cut, exposed for frozen-reference regression tests. */
  ...
  tokenize(input: string): string[] {
    const text = input.normalize('NFKC'), result: string[] = [];
    const other = (chunk: string) => {
      const cleaned = chunk.replace(/[\p{P}\p{S}]/gu, ' ').replace(SPACE, ' ').replace(/ +/g, ' ').trim();
      if (cleaned) for (const piece of this.cut(cleaned)) { const token = this.clean(piece); if (token) result.push(token); }
    };
    let cursor = 0;
    for (const match of text.matchAll(LATIN)) {
      other(text.slice(cursor, match.index)); result.push(...expandSpecialTerm(match[0]));
      cursor = match.index + match[0].length;
    }
    other(text.slice(cursor)); return result;
  }
}
