---
name: ask-matt
description: 询问哪种 skill 或流程适合你的场景。是本仓库中用户调用类 skill 的路由器。
...
- **`/codebase-design`** — the deep-modulary (module, interface, depth, seam, adapter, leverage, locality) for designing a module's *shape*: a lot of behaviour behind a small interface at a clean seam. `/tdd` and `/improve-codebase-architecture` both speak it.
