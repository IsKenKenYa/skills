...
description: 在 Compose Android 中使用 NavigationEvent（androidx.navigationevent）库拦截返回手势并运行预测性返回（Predictive Back）动画。处理 Activity 配置、`ViewPagers` 或标签页中的父子分发器作用域、Compose `NavigationBackHandler`，以及 SDK 36+ 上从旧版 `BackHandler` 的迁移。
