...
description: Android Intent 安全最佳实践。当审计 AndroidManifest.xml 中的组件配置（activity、service、receiver），或审计处理传入 Intent（getIntent、getParcelableExtra）的源代码以防止 Intent 重定向和未授权访问时使用。
